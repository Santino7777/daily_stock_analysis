...
**解决方案**：
1. 已在 v2.3.0 修复，系统现在支持美股代码自动识别（纯字母 ticker 如 `AAPL`、`MSFT`、`META`、`BMNR`、`CIFR`，以及 `BRK.B`、`BRK-B` 等带 `.`/`-` 的类股代码）
2. **v2.4.0 起**：系统自动对美股 ticker 优先使用 Yahoo Finance (yfinance) 数据源，无需手动配置
3. 如需覆盖优先级（例如全局优先 yfinance），可在 `.env` 中设置：
   ```bash
   YFINANCE_PRIORITY=0
   ```
   这将在所有股票（不仅限于美股）中优先使用 Yahoo Finance 数据源
